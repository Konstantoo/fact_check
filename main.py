...
class TelegramFactCheckerBot:
    ...
    async def start_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        welcome_text = f"""
🔍 **Добро пожаловать в бота проверки фактов!**

Привет, {username}! Я помогу вам проверить достоверность информации и проанализировать статьи.
    ...
    async def handle_article_analysis(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            if is_free:
                ...
                explanation_text += "✅ **Одна бесплатная попытка**, далее стоимость 449₽ за исследование"
            else:
                explanation_text += "🔬 **Deep Research** - углубленный анализ с дополнительными источниками\n"
...
**Выберите действие:**
    ...
    async def handle_deep_research(self, query, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not context.user_data.get('last_analysis'):
            await query.edit_message_text(
                "❌ Нет данных для Deep Research. Сначала проведите обычный анализ статьи.",
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton("🔙 Главное меню", callback_data="main_menu")
                ]])
            )
    ...
    async def confirm_deep_research(self, query, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            await query.edit_message_text(
                "🔬 **Запускаю Deep Research...**\n\n"
                "⏱️ **Ожидаемое время:** 2-3 минуты\n"
                "🧠 **Модель:** sonar-deep-research (премиум)\n"
                "📊 **Анализ:** углубленное исследование с экспертами\n\n"
