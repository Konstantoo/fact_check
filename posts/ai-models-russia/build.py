...
for fname,(title,sections,with_call) in parts.items():
  ...
  footer.dots{{color:var(--muted);font-size:26px;margin-top:34px;padding-top:22px;border-top:1px solid var(--line)}}
  footer.dots b{{color:var(--text)}}
  footer.dots .dot{{width:22px;height:22px;display:inline-block;vertical-align:-3px;margin-right:10px;box-shadow:none}}
  footer.dots .g{{background:var(--green)}} footer.dots .r{{background:var(--red)}}
  ...
  {sections}
    <div class="call"><b>Прогноз автора:</b> Qwen скоро выстрелит. Это единственная модель уровня топ-3, которая открывается из России напрямую и без регистрации по иностранному номеру, а картинки, код и агенты уже в одном чате.</div>
    <footer class="dots"><span class="dot g"></span><b>Зелёный</b> — открывается из России напрямую, сразу.&nbsp;&nbsp;<span class="dot r"></span><b>Красный</b> — напрямую не открывается, нужен обходной путь.</footer>
